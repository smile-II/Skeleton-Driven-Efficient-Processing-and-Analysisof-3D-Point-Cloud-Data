...
def query_ball2(centroid, xyz, radius, k):
    B, N, _ = xyz.shape
    _, S, _ = centroid.shape
    device = xyz.device
    group_idx = torch.arange(N, dtype=torch.long).to(device).view(1,1,N).repeat([B, S, 1])  # [B,S,N]
    distance, _ = calculate_distance(centroid, xyz)  # [B,S,N]
    radius = radius * 1.2  # [B,S]
    radius = radius.unsqueeze(2)  # [B,S,1]
    group_idx[distance > radius] = N
    group_idx = group_idx.sort(dim=-1)[0][:,:,:k]
...
def knn_gyg2(p1_xyz, p2_xyz, p1_normal, p2_normal, k):
    """
    :param p1: size[B,N,D]
    :param p2: size[B,M,D]
    :param k: k nearest neighbors
    ...
    measure = normalized_similarity - normalized_distance
    radius_point_idx = torch.argmax(measure, dim=-1)
    relative_point = index_points(p2_xyz, radius_point_idx)
    centroid = (p1_xyz + relative_point) / 2
    query_ball_radius = distance[torch.arange(B).unsqueeze(1), torch.arange(N), radius_point_idx]  # [B,N]
    group_idx = query_ball2(centroid, p2_xyz, query_ball_radius, k)
    return group_idx

def query_ball3(centroid, xyz, radius, k):
    B, N, _ = xyz.shape
    _, S, _ = centroid.shape
    device = xyz.device
    group_idx = torch.arange(N, dtype=torch.long).to(device).view(1,1,N).repeat([B, S, 1])  # [B,S,N]
    distance, _ = calculate_distance(centroid, xyz)  # [B,S,N]
    radius = radius * 1.3  # [B,S]
    radius = radius.unsqueeze(2)  # [B,S,1]
    group_idx[distance > radius] = N
    group_idx = group_idx.sort(dim=-1)[0][:,:,:k]
    group_first = group_idx[:,:,0].view(B, S, 1).repeat([1, 1, k])
...
def knn_gyg3(p1_xyz, p2_xyz, p1_normal, p2_normal, k):
    """
    :param p1: size[B,N,D]
    :param p2: size[B,M,D]
    :param k: k nearest neighbors
    ...
    centroid = (p1_xyz + relative_point) / 2
    query_ball_radius = distance[torch.arange(B).unsqueeze(1), torch.arange(N), radius_point_idx]  # [B,N]
    group_idx = query_ball3(centroid, p2_xyz, query_ball_radius / 2, k)
    return group_idx
