...
def closest_distance_with_batch(p1, p2, is_sum=True):
    ...
    p1 = p1.unsqueeze(1)  # [B, 1, N, D]
    p2 = p2.unsqueeze(1)  # [B, 1, M, D]

    p1 = p1.repeat(1, p2.size(2), 1, 1)  # [B, M, N, D]
    p1 = p1.transpose(1, 2)  # [B, N, M, D]
    p2 = p2.repeat(1, p1.size(1), 1, 1)  # [B, N, M, D]

    dist = torch.add(p1, torch.neg(p2))
    dist = torch.norm(dist, 2, dim=3)  # [B, N, M]

    min_dist, min_indice = torch.min(dist, dim=2)  # min_dist:[B, N]
