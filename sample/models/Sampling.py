...
class GumbelSampling(nn.Module):
    ...
    def forward(self, x, tau=1., hard=False, mask=None):  # x:[B,N,D]
        sample_weights = self.sampler(x)  # [B,N,S]
...
class InfoSampling(nn.Module):
    ...
    def forward(self, x, tau, hard, prior_weight):  # x:[B,C,N]  prior_weight:[B,N]
        batch_size = x.size(0)
        num_points = x.size(2)
        ...
        x = x * prior_weight.unsqueeze(1)  # [B,256,num_points]
        x = self.dp(x)
        sampled, attn = self.sampler(x.transpose(1, 2), tau=tau, hard=hard)  # sampled [B, 256, args.num_sampling]  attn [B, args.num_sampling, N]
